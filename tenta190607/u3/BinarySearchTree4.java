...
public class BinarySearchTree4 <E extends Comparable<E>> {
    ...
    private int nrNodesWith2children(Node<E> node){
        if(node == null) return 0;
        if(nrChildren(node) == 2) return 1 + nrNodesWith2children(node.left) + nrNodesWith2children(node.right);
    ...
    private int nrNodesWith2grandchildren(Node<E> node){
        if(node == null) return 0;
        if(nrChildren(node.left) + nrChildren(node.right) == 2) return 1 + nrNodesWith2grandchildren(node.left) + nrNodesWith2grandchildren(node.right);
