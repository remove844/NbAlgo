...
public class BinarySearchTree2 <E extends Comparable<E>> {
    ...
    private E findClosestEqualOrLarger(Node<E> node, E target){
        if(node == null) return null;
        if(target.compareTo(node.data) > 0) return findClosestEqualOrLarger(node.right, target);
        if(target.compareTo(node.data) == 0) return node.data;

        E leftData = findClosestEqualOrLarger(node.left, target);
    ...
    public static void buildTree(BinarySearchTree2<Integer> b){
        b.add(10);
        b.add(5);
        b.add(15);
        b.add(2);
